...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (forbidden.some((key) => key in body)) {
      return json({
        ok: false,
        error: 'Pusula Deneyi kanıtları istemciden kabul edilmez; yalnız field_id gönderilebilir.',
      }, 400);
    }
    ...
    return json({
      ...
      user_visible: false,
      internal_only: true,
      note: 'Pusula Deneyi yalnız iç kanıt değerlendirmesidir. Model sıralamaz, modeli otomatik terfi ettirmez ve çiftçi kararını değiştirmez.',
    });
  } catch (error) {
